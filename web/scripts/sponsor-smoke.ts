 ...
 *
 * Enoki portal setup:
 *   1. Allowlist `${PACKAGE_ID}::ticketing::register_issuer` (and the other
 *      three SPONSORED_TARGETS) under Sponsored Transactions.
...
const PACKAGE_ID =
  process.env.NEXT_PUBLIC_SUI_TICKET_PACKAGE_ID ??
...
async function main() {
  ...
  const cap = tx.moveCall({
    target: `${PACKAGE_ID}::ticketing::register_issuer`,
    arguments: [
      tx.pure.string("Acme Sponsored Smoke"),
      tx.pure.vector("u8", new TextEncoder().encode("sponsored=true")),
    ],
  });
  ...
  const sponsored = await enoki.createSponsoredTransaction({
    ...
    sender: SENDER,
    allowedMoveCallTargets: [`${PACKAGE_ID}::ticketing::register_issuer`],
  });
