...
fun post_review_with_matching_poap_ok() {
    ...
    reviews::post_review(&ev, &poap, 5, s(b"blob-review-1"), &clock, sc.ctx());
...
// Boundary ratings 1 and 5 are both valid (inclusive range).
#[test]
fun post_review_boundary_ratings_ok() {
    ...
    let ev = ts::take_shared_by_id<Event>(&sc, eid);
    let poap = sc.take_from_sender<Poap>();
    reviews::post_review(&ev, &poap, 1, s(b"blob-min"), &clock, sc.ctx());
    reviews::post_review(&ev, &poap, 5, s(b"blob-max"), &clock, sc.ctx());
    sc.return_to_sender(poap);
    ts::return_shared(ev);
...
fun post_review_wrong_event_poap_aborts() {
    ...
    sc.next_tx(BUYER);
    let ev_b = ts::take_shared_by_id<Event>(&sc, eid_b);
    let poap = sc.take_from_sender<Poap>();
    reviews::post_review(&ev_b, &poap, 4, s(b"blob-x"), &clock, sc.ctx());
    sc.return_to_sender(poap);
...
fun post_review_rating_zero_aborts() {
    ...
    let ev = ts::take_shared_by_id<Event>(&sc, eid);
    let poap = sc.take_from_sender<Poap>();
    reviews::post_review(&ev, &poap, 0, s(b"blob-0"), &clock, sc.ctx()); // 0 < MIN_RATING
    sc.return_to_sender(poap);
    ts::return_shared(ev);
...
fun post_review_rating_six_aborts() {
    ...
    let ev = ts::take_shared_by_id<Event>(&sc, eid);
    let poap = sc.take_from_sender<Poap>();
    reviews::post_review(&ev, &poap, 6, s(b"blob-6"), &clock, sc.ctx()); // 6 > MAX_RATING
    sc.return_to_sender(poap);
    ts::return_shared(ev);
