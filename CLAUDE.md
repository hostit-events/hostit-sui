...
- `event` — `create_event` shares one `Event` per event and gives the creator an `OrganizerCap{event_id}`; `create_event_with_price<T>` does create+price atomically (#68, so a paid event is never left priced-less). Holds per-coin price + escrow via dynamic fields. Organizer edits via `update_metadata`/`update_times`/`update_max_tickets`/`update_max_per_user` (all cap-gated). Public getters (`minted`, `start_ms`, `max_tickets`, `event_seq`, …) are read by other modules.
