...
|---|---|---|
| `PACKAGE_ID` | `0x80ffb7c9…e3e3f0` (fresh v1) | type identity for core modules |
| `PREDICT_SELLOUT_PKG` | `0x80ffb7c9…e3e3f0` (= v1) | `SelloutMarket` type/events |
| `PACKAGE_ID_LATEST` | `0x80ffb7c9…e3e3f0` (= v1) | **all `predict` calls** + `RangeMarket` type/events |

> Fresh publish (2026-06-20): all pins equal `0x80ffb7c9…e3e3f0` until the first in-place upgrade, which re-splits them (latest rolls forward, type origins stay).
...
- The deployer address holds the package **`UpgradeCap`** `0xc6a144fb…b785d` and enough gas (`sui client gas`).
...
flowchart TD
  ...
  B --> C["sui client upgrade<br/>--upgrade-capability 0xc6a144fb…<br/>(Published.toml tracks published-at)"]
...
2. **Upgrade** (from the repo root). With automated address management, `Published.toml` supplies `published-at`; pass the cap explicitly:
   ```bash
   sui client upgrade --upgrade-capability 0xc6a144fbe32e024a61ed9ff1723e5c3bee99fdc53f6c7a31fa20d112d67b785d \
